    ...
    try:
        if connection.is_connected():
            ...
            try:
                ...
                nt = st.number_input(
                    'Enter the no of tickets: ', step=1, min_value=1)
                    ...
                    try:
                        ...
                        if available_tickets >= nt:
                            ...
                            if result_user:
                                ...
                                # df1 = pd.DataFrame(result_tickets_user, columns=['User ID', 'Tour Date', 'Ticket Booking date', 'No of Tickets'])
                                # st.header("Ticket Booked details")
                                # st.write(df1)

                                current_timestamp = datetime.now()
                                current_timestamp = current_timestamp.strftime(
                                    "%Y-%m-%d %H:%M:%S")
                            ...
                            st.write(
                                "No tickets available for the given booking date!!")
